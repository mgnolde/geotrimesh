...
def coord_to_pix(
    x, y, rast_xmin, rast_ymax, rast_xres, rast_yres, cols, rows, offset_x, offset_y
):
...
def generate_features(
    in_dtm_filepath,
    in_dom_filepath,
    in_scad_dtm_filepath,
    in_scad_dom_filepath,
    in_clippoly_filepath,
    in_mesh_dtm_filepath,
    in_mesh_dom_filepath,
    in_feat_filepath,
    asset_dirpath,
    zmean_total,
    task,
    ...
    in_mesh_dtm_filename_base, in_mesh_dtm_filename_ext = os.path.splitext(
        in_mesh_dtm_filename
    )

    in_mesh_dom_dirpath = os.path.dirname(in_mesh_dom_filepath)
    in_mesh_dom_filename = os.path.basename(in_mesh_dom_filepath)
    in_mesh_dom_filename_base, in_mesh_dom_filename_ext = os.path.splitext(
        in_mesh_dom_filename
    )

    try:
        in_mesh_dtm_filename_base_prefix = "_".join(
            in_mesh_dtm_filename_base.split("_")[0:-2]
        )
        in_mesh_dom_filename_base_prefix = "_".join(
            in_mesh_dom_filename_base.split("_")[0:-2]
        )
    ...
    in_mesh_neighbours_filenames_list.append(
        [
            in_mesh_dtm_filename_base_prefix
            + "_"
            + str(in_mesh_tiley - 1)
            + "_"
            + str(in_mesh_tilex - 1),
            in_mesh_dom_filename_base_prefix
            + "_"
            + str(in_mesh_tiley - 1)
            + "_"
            + str(in_mesh_tilex - 1),
    ...
    in_clippoly_layer_extent_xcent = (
        in_clippoly_layer_extent_xmax + in_clippoly_layer_extent_xmin
    ) / 2.0
    in_clippoly_layer_extent_ycent = (
        in_clippoly_layer_extent_ymax + in_clippoly_layer_extent_ymin
    ) / 2.0
    ...
    in_mesh_dtm_ring.AddPoint(
        in_mesh_dtm_xmin + in_clippoly_layer_extent_xcent,
        in_mesh_dtm_ymin + in_clippoly_layer_extent_ycent,
    )
    in_mesh_dtm_ring.AddPoint(
        in_mesh_dtm_xmax + in_clippoly_layer_extent_xcent,
        in_mesh_dtm_ymin + in_clippoly_layer_extent_ycent,
    )
    in_mesh_dtm_ring.AddPoint(
        in_mesh_dtm_xmax + in_clippoly_layer_extent_xcent,
        in_mesh_dtm_ymax + in_clippoly_layer_extent_ycent,
    )
    in_mesh_dtm_ring.AddPoint(
        in_mesh_dtm_xmin + in_clippoly_layer_extent_xcent,
        in_mesh_dtm_ymax + in_clippoly_layer_extent_ycent,
            ...
            if geom_name.lower() == "multipolygon":
                ...
                for sub_id in range(0, geom.GetGeometryCount()):
                    ...
                    for bound_id in range(0, geom_sub.GetGeometryCount()):
                        ...
                        for point_id in range(0, geom_sub_bound.GetPointCount()):
                            ...
                            (
                                geom_sub_bound_point_x_tmp,
                                geom_sub_bound_point_y_tmp,
                                dummy,
                            ...
                            geom_sub_minx = (
                                ...
                                if (
                                    not geom_sub_minx
                                    or geom_sub_bound_point_x < geom_sub_minx
                                )
                                else geom_sub_minx
                            )
                            geom_sub_maxx = (
                                ...
                                if (
                                    not geom_sub_maxx
                                    or geom_sub_bound_point_x > geom_sub_maxx
                                )
                                else geom_sub_maxx
                            )
                            geom_sub_miny = (
                                ...
                                if (
                                    not geom_sub_miny
                                    or geom_sub_bound_point_y < geom_sub_miny
                                )
                                else geom_sub_miny
                            )
                            geom_sub_maxy = (
                                ...
                                if (
                                    not geom_sub_maxy
                                    or geom_sub_bound_point_y > geom_sub_maxy
                                )
                                else geom_sub_maxy
                            )
                            ...
                            if geom_sub_bound_dtm_point_z:
                                ...
                                geom_sub_dtm_minz = (
                                    geom_sub_bound_dtm_point_z
                                    if (
                                        not geom_sub_dtm_minz
                                        or geom_sub_bound_dtm_point_z
                                        < geom_sub_dtm_minz
                                    )
                                    else geom_sub_dtm_minz
                                )
                                geom_sub_dtm_maxz = (
                                    geom_sub_bound_dtm_point_z
                                    if (
                                        not geom_sub_dtm_maxz
                                        or geom_sub_bound_dtm_point_z
                                        > geom_sub_dtm_maxz
                                    )
                                    else geom_sub_dtm_maxz
                                )
                                geom_sub_dom_minz = (
                                    geom_sub_bound_dom_point_z
                                    if (
                                        not geom_sub_dom_minz
                                        or geom_sub_bound_dom_point_z
                                        < geom_sub_dom_minz
                                    )
                                    else geom_sub_dom_minz
                                )
                                geom_sub_dom_maxz = (
                                    geom_sub_bound_dom_point_z
                                    if (
                                        not geom_sub_dom_maxz
                                        or geom_sub_bound_dom_point_z
                                        > geom_sub_dom_maxz
                                    )
                                    else geom_sub_dom_maxz
                                )
                            ...
                            polygon_points_geom.append(
                                [geom_sub_bound_point_x, geom_sub_bound_point_y]
                            )
                            point_cnt += 1
                    ...
                    if len(polygon_paths) > 0:
                        ...
                        feat_line = "    translate([0,0,{}]) linear_extrude(height={}) polygon({},{});".format(
                            geom_sub_dtm_minz_field,
                            geom_sub_dom_maxz_field - geom_sub_dtm_minz_field,
                            polygon_points,
                            polygon_paths,
                        )
                        feat_line2 = (
                            ...
                            + "translate([0,0,{}]) linear_extrude(height={}) polygon({},{});".format(
                                -9999, 20000, polygon_points, polygon_paths
                            )
                            + " dem(); }"
                        )
                        feat_line3 = (
                ...
                for bound_id in range(0, geom.GetGeometryCount()):
                    ...
                    for point_id in range(0, geom_bound.GetPointCount()):
                        ...
                        (
                            geom_bound_point_x_tmp,
                            geom_bound_point_y_tmp,
                            dummy,
                        ) = geom_bound.GetPoint(point_id)
                        # print("Coord", geom_bound_point_x_tmp, geom_bound_point_y_tmp)

                        # print("Centr", geom_bound_point_x_tmp, geom_bound_point_y_tmp)

                        geom_bound_point_x = (
                            geom_bound_point_x_tmp - in_clippoly_layer_extent_xcent
                        )
                        geom_bound_point_y = (
                            geom_bound_point_y_tmp - in_clippoly_layer_extent_ycent
                        )
                        ...
                        geom_minx = (
                            geom_bound_point_x
                            if (not geom_minx or geom_bound_point_x < geom_minx)
                            else geom_minx
                        )
                        geom_maxx = (
                            geom_bound_point_x
                            if (not geom_maxx or geom_bound_point_x > geom_maxx)
                            else geom_maxx
                        )
                        geom_miny = (
                            geom_bound_point_y
                            if (not geom_miny or geom_bound_point_y < geom_miny)
                            else geom_miny
                        )
                        geom_maxy = (
                            geom_bound_point_y
                            if (not geom_maxy or geom_bound_point_y > geom_maxy)
                            else geom_maxy
                        )
                        ...
                        if geom_bound_dtm_point_z:
                            ...
                            geom_dtm_minz = (
                                geom_bound_dtm_point_z
                                if (
                                    not geom_dtm_minz
                                    or geom_bound_dtm_point_z < geom_dtm_minz
                                )
                                else geom_dtm_minz
                            )
                            geom_dtm_maxz = (
                                ...
                                if (
                                    not geom_dtm_maxz
                                    or geom_bound_dtm_point_z > geom_dtm_maxz
                                )
                                else geom_dtm_maxz
                            )
                            geom_dom_minz = (
                                geom_bound_dom_point_z
                                if (
                                    not geom_dom_minz
                                    or geom_bound_dom_point_z < geom_dom_minz
                                )
                                else geom_dom_minz
                            )
                            geom_dom_maxz = (
                                ...
                                if (
                                    not geom_dom_maxz
                                    or geom_bound_dom_point_z > geom_dom_maxz
                                )
                                else geom_dom_maxz
                            )
                        ...
                        polygon_path.append(point_cnt)
                        polygon_points_geom.append(
                            [geom_bound_point_x, geom_bound_point_y]
                        )
                        point_cnt += 1
                ...
                if task.lower() == "extrude":
                    ...
                    if len(polygon_paths) > 0:
                        feat_line = "    translate([0,0,{}]) linear_extrude(height={}) polygon({},{});".format(
                            geom_dtm_minz_field,
                            geom_dom_maxz_field - geom_dtm_minz_field,
                            polygon_points,
                            polygon_paths,
                        )
                        feat_line2 = (
                ...
                if task.lower() == "insert":
                    ...
                    if len(polygon_paths) > 0:
                        feat_line = '     translate([{},{},{}]) rotate([90,0,0]) import("{}", convexity=10);'.format(
                            centroid_x, centroid_y, geom_dtm_minz_field, asset_filepath
                        )
                        # c.append(feat_line)
                        c2.append(feat_line)  ## output extruded polygon
...
in_clippoly_filepath = os.path.join(
    os.sep,
    "mnt",
    "e",
    "zh",
    "wambachers_osm__boundaries__adm",
    "data",
    "district_zurich_al6_al6_2056.shp",
)
in_mesh_dtm_filepath = os.path.join(
    os.sep,
    "mnt",
    "e",
    "zh",
    "gis_zh__dom_dtm__lidar",
    "dtm_mosaic_tiled_offset_500x500_scad",
    "dtm_mosaic_tiled_offset_500x500_11_18.stl",
)
in_mesh_dom_filepath = os.path.join(
    os.sep,
    "mnt",
    "e",
    "zh",
    "gis_zh__dom_dtm__lidar",
    "dom_mosaic_tiled_offset_500x500_scad",
    "dom_mosaic_tiled_offset_500x500_11_18.stl",
)
...
mold_command_lines, feat_command_lines = generate_features(
    in_dtm_filepath,
    in_dom_filepath,
    in_scad_dtm_filepath,
    in_scad_dom_filepath,
...
if task == "extrude":
    if len(mold_command_lines) > 0:
        with open(
            os.path.join(scad_dirpath, scad_dtm_filename_base + ".scad"), "w"
        ) as scad_file:
...
if task == "extrude":
    with open(
        os.path.join(scad_dirpath, scad_feat_filename_base + ".scad"), "w"
    ) as scad_file:
...
if task == "insert":
    for feat_command_line_id, feat_command_line in enumerate(feat_command_lines):
        with open(
            os.path.join(
                scad_dirpath,
                scad_feat_filename_base + str(feat_command_line_id) + ".scad",
            ),
            "w",
        ) as scad_file:
            scad_file.write(feat_command_line + "\n")
...
if not proc_target_os == "win":
    ...
    subprocess_commands = [
        subprocess_bin,
        "-o",
        os.path.join(proc_dirpath, scad_filename_base + ".stl"),
        os.path.join(scad_dirpath, scad_filename_base + ".scad"),
    ]
    output = subprocess.check_output(subprocess_commands, shell=False)

else:

    openscad_win_bin_filepath = "\\".join(
        ["C:", '"Program Files"', "OpenSCAD", "openscad.com"]
    )
    openscad_linux_bin_filepath = os.path.join(
        os.sep,
        "home",
        "mic",
        "prog",
        "OpenSCAD-2019.05-x86_64",
        "squashfs-root",
        "usr",
        "bin",
        "openscad",
    ...
    subprocess_dtm_args = [
        "-o",
        scad_dtm_filename_base + ".stl",
        scad_dtm_filename_base + ".scad",
    ]
    subprocess_feat_args = [
        "-o",
        scad_feat_filename_base + ".stl",
        scad_feat_filename_base + ".scad",
    ]
    ...
    with open(
        os.path.join(scad_dirpath, scad_dtm_filename_base + ".bat"), "w"
    ) as batch_file:
        batch_file.write(" ".join(subprocess_win_dtm_command) + "\n")
        batch_file.write(" ".join(subprocess_win_feat_command) + "\n")

    with open(
        os.path.join(scad_dirpath, scad_dtm_filename_base + ".sh"), "w"
    ) as shell_file:
